# Seller interface to handle everything for client side ================================================================
...
from sub_files.classes import Sale, Upgrade
...
class FrontEndTwo:
    ...
    def __init__(self, window):
        ...
        self.root.title("RUSH LTD")
        self.root.geometry("1200x672+0+0")
        self.root.resizable(False, False)

        self.title = Label(self.root, text="RUSH LTD", bd=4, relief=GROOVE,
        ...
        self.car_showall_btn = Button(self.car_frame, text="Show All", width=10, command=self.car_showall).\
            grid(row=0, column=7, padx=20, pady=10)
        ...
        self.accessory_clear_btn = Button(self.manage_frame, text="Clear", width=10, command=self.clear_list_item).\
            grid(row=0, column=2, padx=10, pady=10)
        self.accessory_removeall_btn = Button(self.manage_frame, text="Remove All", width=10,
                                              command=self.removeall_list_item).grid(row=0, column=3, padx=10, pady=10)
        self.accessory_add_btn = Button(self.manage_frame, text="Add", width=10,
                                        command=self.add_list_item).grid(row=1, column=2, padx=10, pady=10)
        self.accessory_remove_btn = Button(self.manage_frame, text="Remove", width=10,
                                           command=self.remove_list_item).grid(row=1, column=3, padx=10, pady=10)
        ...
        self.msg_lbl = Label(self.price_frame,
                             text="If you are ready to buy this vihicle with selected upgrades, click 'PROCEED'",
        ...
        self.currency_combobox.bind("<<ComboboxSelected>>", self.currency_select)
            ...
            if search_by == "Reg.No":
                if condition == "":
                    self.car_table.insert("", tk.END, values=(
                        car.getRegNo(), car.getColor(), car.getNoOfDoors(), car.getModel().getManufacturer().getId(),
                        car.getModel().getManufacturer().getName(), car.getModel().getModelNo(),
                        car.getModel().getName(),
                        self.convert_currency(car.getModel().getPrice())))
                else:
                    if car.getRegNo().find(condition) != -1:
                        self.car_table.insert("", tk.END, values=(
                            car.getRegNo(), car.getColor(), car.getNoOfDoors(),
                            car.getModel().getManufacturer().getId(),
                            car.getModel().getManufacturer().getName(), car.getModel().getModelNo(),
                            car.getModel().getName(),
                            self.convert_currency(car.getModel().getPrice())))
                ...
                if condition == "":
                    ...
                    if car.getModel().getName().find(condition) != -1:
                        self.car_table.insert("", tk.END, values=(
                            car.getRegNo(), car.getColor(), car.getNoOfDoors(),
                            car.getModel().getManufacturer().getId(),
                            car.getModel().getManufacturer().getName(), car.getModel().getModelNo(),
                ...
                if condition == "":
                    ...
                    if car.getModel().getManufacturer().getId().find(condition) != -1:
                        self.car_table.insert("", tk.END, values=(
                            car.getRegNo(), car.getColor(), car.getNoOfDoors(),
                            car.getModel().getManufacturer().getId(),
                            car.getModel().getManufacturer().getName(), car.getModel().getModelNo(),
                ...
                if condition == "":
                    ...
                    if car.getModel().getManufacturer().getName().find(condition) != -1:
                        self.car_table.insert("", tk.END, values=(
                            car.getRegNo(), car.getColor(), car.getNoOfDoors(),
                            car.getModel().getManufacturer().getId(),
                            car.getModel().getManufacturer().getName(), car.getModel().getModelNo(),
                ...
                self.car_table.insert("", tk.END, values=(
                    car.getRegNo(), car.getColor(), car.getNoOfDoors(), car.getModel().getManufacturer().getId(),
                    car.getModel().getManufacturer().getName(), car.getModel().getModelNo(), car.getModel().getName(),
                    self.convert_currency(car.getModel().getPrice())))
        ...
        for accessory in self.accessories_list:
            if condition == "":
                self.accessory_table.insert("", tk.END, values=(accessory.getId(), accessory.getName(),
                                                                self.convert_currency(accessory.getPrice())))
            else:
    ...
    def set_list_data(self):
        ...
        if self.selected_car != None:
            self.list_table.insert("", tk.END, values=(self.selected_car.getRegNo(), 1,
                                                       self.convert_currency(self.selected_car.getModel().getPrice())))
        for a in self.selected_accessories:
            self.list_table.insert("", tk.END, values=(a[0].getId(), a[1], self.convert_currency(a[0].getPrice())))
    ...
    def set_total_price(self):
        ...
        self.price_lbl.config(text="{:.2f}".format(self.convert_currency(self.total_price))+" ("+
                                   self.currency_combobox_txt.get()+")")

    def convert_currency(self, price):
        type = self.currency_combobox_txt.get()
        if type=="Dollar":
            price = price*1.34
        elif type=="Euro":
            price = price*1.11
    ...
    def currency_select(self, event):
        self.set_car_data()
        self.set_accessory_data()
        self.set_list_data()
    ...
    def proceed(self):
        if self.selected_car != None:
            ...
            sale = Sale(self.selected_car.getRegNo(),now.strftime("%Y-%m-%d %H:%M:%S"), self.total_price)
            upgrades = []
            for item in self.selected_accessories:
                up = Upgrade(self.selected_car.getRegNo(), item[0].getId(), item[1])
                upgrades.append(up)

            msg = self.backend.addSale(sale, upgrades)
            if msg=="Done":
                self.removeall_list_item()
                self.popupmsg("Proceed successfully")
            else:
                self.popupmsg(msg)
