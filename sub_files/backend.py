...
class Backend:
    ...
    def createTables(self):
        try:
            ...
            self.cursor.execute(
                "CREATE TABLE upgrade(car_reg_no TEXT, accessory_id TEXT NOT NULL, quantity INTEGER NOT NULL, "
    ...
    def getModel(self, model_no):
        ...
        try:
            ...
            rows = self.cursor.fetchall()
            for row in rows:
                model = Model(row[0], row[1], row[2], self.getManufacturer(row[3]))
        except Error as e:
            return model
        return model
    ...
    def addModel(self, model):
        ...
        try:
            self.cursor.execute("INSERT INTO model VALUES(?,?,?,?)",(model.getModelNo(), model.getName(), model.getPrice(), model.getManufacturer().getId()))
    ...
    def updateModel(self, model):
        ...
        if con1 or con2 or con3 or con4:
            return "you have missed something"
        try:
            self.cursor.execute("UPDATE model SET name = '" + model.getName() + "', price = " +
                                str(model.getPrice()) + ", manufacturer = '"+
                                model.getManufacturer().getId()+"' WHERE model_no = '" +
                                model.getModelNo() + "'")
            self.connection.commit()
            return "Done"
        except Error as e:
    ...
    def addCar(self, car):
        ...
        if con1 or con2 or con3 or con4:
            return "you have missed something"
        try:
            self.cursor.execute("INSERT INTO car VALUES(?,?,?,?)", (car.getRegNo(), car.getColor(), car.getNoOfDoors(), car.getModel().getModelNo()))
            self.connection.commit()
            return "Done"
        except Error as e:
            return e
    ...
    def addSale(self, sale, upgrades):
        con1 = sale.getCarRegNo() == None or sale.getCarRegNo().strip() == ""
        con2 = sale.getTimestamp() == None
        con3 = sale.getFinalAmount() == None or sale.getFinalAmount == 0
        if con1 or con2 or con3:
            return "you have missed something"
        try:
            self.cursor.execute("INSERT INTO sale VALUES(?,?,?)",
                                (sale.getCarRegNo(), sale.getTimestamp(), sale.getFinalAmount()))

            for up in upgrades:
                self.cursor.execute("INSERT INTO upgrade VALUES(?,?,?)",
                                    (up.getCarRegNo(), up.getAccessoryId(), up.getQuantity()))
            self.cursor.execute("DELETE FROM car WHERE reg_no='" + sale.getCarRegNo() + "'")
            self.connection.commit()
            return "Done"
        except Error as e:
            return e
    ...
    def getUpgrades(self, id):
        upgrades = []
        try:
            self.cursor.execute("SELECT * FROM upgrade WHERE car_reg_no='"+id+"'")
            rows = self.cursor.fetchall()
            for row in rows:
